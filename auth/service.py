...
def get_user_via_email(email: str) -> dict | None:
    ...
    if user:
        # Note: This returns the *serialized* user, not the raw doc with the password.
        # For login, we need the raw doc with the password, which is done directly in main.py's /login endpoint.
