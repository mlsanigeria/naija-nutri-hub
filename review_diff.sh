...
# PR branch checkout - supports multiple methods
if [ -n "$1" ]; then
    # Method 1: Branch name passed as argument
    PR_BRANCH="$1"
    echo "Checking out PR branch: $PR_BRANCH"
    git checkout "$PR_BRANCH" || { echo "Failed to checkout branch $PR_BRANCH"; exit 1; }
elif [ -n "$GITHUB_HEAD_REF" ]; then
    # Method 2: GitHub Actions environment variable
    PR_BRANCH="$GITHUB_HEAD_REF"
    echo "Checking out PR branch from GitHub Actions: $PR_BRANCH"
    git checkout "$PR_BRANCH" || { echo "Failed to checkout branch $PR_BRANCH"; exit 1; }
elif [ -n "$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME" ]; then
    # Method 3: GitLab CI environment variable
    PR_BRANCH="$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME"
    echo "Checking out PR branch from GitLab CI: $PR_BRANCH"
    git checkout "$PR_BRANCH" || { echo "Failed to checkout branch $PR_BRANCH"; exit 1; }
else
    # Method 4: Stay on current branch
    PR_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    echo "No PR branch specified, using current branch: $PR_BRANCH"
fi
...
echo "Review complete! Check $REPORT_FILE"
